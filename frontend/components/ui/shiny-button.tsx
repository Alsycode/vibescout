...
const SHINY_STYLES = `
  ...
  .shiny-cta span::before {
    ...
    animation: calc(var(--duration) * 1.5) breathe-shiny linear infinite;
  }
  ...
  @keyframes shimmer-shiny  { to { rotate: 360deg; } }
  @keyframes breathe-shiny  {
    from, to { scale: 1; }
    50%       { scale: 1.2; }
  }
...
export function ShinyButton({ children, onClick, href, className = "" }: ShinyButtonProps) {
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: SHINY_STYLES }} />
      {href ? (
