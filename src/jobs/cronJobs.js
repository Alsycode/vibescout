...
// PURPOSE: Scheduled cron jobs to refresh cluster-level environment signals on
// defined cadences.
// PERF-3 — this used to schedule on import, which meant every process that
// imported this file (i.e. every autoscaled API instance) ran every job:
// N× external API spend and racing writes to the same Cluster docs. Now it's
// an explicit init, called only from worker.js — never from server.js — and
// each job takes a Redis lock before running so it's still safe even if the
// worker process is itself ever scaled to more than one instance.

import cron from 'node-cron';
import Cluster from '../models/Cluster.js';
import { getClustersNeedingRefresh } from '../services/clusterService.js';
import { redisSet } from '../lib/redis.js';
import { withCronLock } from '../lib/cronLock.js';
...
// Generous relative to how long any of these batches actually take (seconds
// to low minutes) but always well clear of the next scheduled run (the
// tightest cadence here is daily) — so a crashed run never blocks tomorrow's.
...
export function initCronJobs() {
  ...
  cron.schedule('0 0 * * *', () => withCronLock('AQI', LOCK_TTL_SECONDS, async () => {
  ...
  cron.schedule('0 0 * * *', () => withCronLock('Weather', LOCK_TTL_SECONDS, async () => {
  ...
  cron.schedule('0 1 * * *', () => withCronLock('Solar', LOCK_TTL_SECONDS, async () => {
  ...
  cron.schedule('0 2 * * 1', () => withCronLock('Noise', LOCK_TTL_SECONDS, async () => {
  ...
  cron.schedule('0 3 1,15 * *', () => withCronLock('Amenities', LOCK_TTL_SECONDS, async () => {
  ...
  console.log('[Cron] Jobs scheduled: AQI, Weather, Solar, Noise, Amenities');
}

// News — daily at 4am (optional — improves cache hit rate; pipeline live-fetch is primary path)
// cron.schedule('0 4 * * *', () => withCronLock('News', LOCK_TTL_SECONDS, async () => {
